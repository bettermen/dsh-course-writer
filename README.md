...
### 三栏式工作台
- **左栏**：章节列表（**拖拽排序** + 悬停删除 + 右键菜单）+ 九阶段门禁导航（进度 x/9、阶段锁定/进行中/已通过状态）
...
### 2️⃣ 编写章节（CodeMirror 编辑器）
- 左栏章节列表点击切换章节；中栏为 **CodeMirror 6 Markdown 编辑器**
...
- 语义说明：
  ...
  - 拖拽**统一重编号为 1..N**，并同步修正一致性账本的课时归属、按新顺序重放课程变量
